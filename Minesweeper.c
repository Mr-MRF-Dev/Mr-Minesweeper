...
void Print_Game(int** Board, struct Player* User, int Size_Board, int Boob_C, int Is_Loss, int Flag_Left, int Debug_Mode);
void Play_Game(struct Player* User, int Size_Board, int Boob_C,int Max_Column_Size);
...
int main() {
    ...
    printf("Warning:\n");
    printf("Ansi Code is used in this program. For a better experience, use a terminal that supports Ansi Code.\n");
...
void Print_Player_Info(struct Player* in) {
    ...
    printf("\n    %sPlayer:%s %s %s\n", Color_Blue, Color_Purple, in->Username, Color_Reset);
    printf("    %sWin: %s%d", Color_Green, Color_Reset, in->Win);
    printf(" %s~ %sLoss: %s%d\n\n", Color_Aqua, Color_Red, Color_Reset, in->Loss);
    printf("    %sCurrent Streak: %s%d\n", Color_Yellow, Color_Reset, in->Cur_Streak);
    printf("    %sWinning Streak: %s%d\n", Color_Yellow, Color_Reset, in->Win_Streak);
    Sleep(3000);
...
void Create_Game(int** Board, int Size_Board, int Boob_C, int Max_Column_Size) {
    ...
    for (int i=0; i<Size_Board; i++) {
        ...
        for (int j=0; j<Size_Board; j++) {
            ...
            if (j+1 < Size_Board && i+1 < Size_Board && Board[i+1][j+1] == 9) Board[i][j]++;

            if (j-1 >= 0 && i-1 >= 0 && Board[i-1][j-1] == 9) Board[i][j]++;

            if (j-1 >= 0 && i+1 < Size_Board && Board[i+1][j-1] == 9) Board[i][j]++;
        
            if (i-1 >= 0 && j+1 < Size_Board && Board[i-1][j+1] == 9) Board[i][j]++;
...
void Play_Game(struct Player* User, int Size_Board, int Boob_C, int Max_Column_Size) {
    ...
    Bar_Status(User, 1);
    printf("%sBoard:%s %d*%d%s, ", Color_Green, Color_Gray, Size_Board, Size_Board, Color_Reset);
    ...
    while( Space_B != 0 || Flag_Left != 0 ) {
        ...
        if (i == -2 || j == -2 || in_c == 3) {
            ...
            printf("\n");
            Bar_Status(User, 1);
            printf("Exit Code (Press Ctrl+C To Exit ~ Press Another Button to Cancel)?  ");
            int exit_game = getch();
